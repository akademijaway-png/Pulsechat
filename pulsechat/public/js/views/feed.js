...
import { pickImage, mediaUrl, openViewer } from '../media.js';
...
function build() {
  return el('div', { class: 'feed fade-swap' }, [
    el('div', { class: 'section-title', text: 'Feed' }),
    composer(),
  ...
  const postBtn = el('button', {
    ...
    onclick: async () => {
      ...
      try {
        ...
        if (picked) form.append('image', picked.file);
        const res = await request('POST', '/feed', { form });
