  ...
  const shell = el('div', { class: 'shell', id: 'shell' }, [
    ...
    el('div', { class: 'col-chat', id: 'colChat' }, [
      el('div', { class: 'chat-empty' }, [
        el('img', { src: '/assets/logo.svg', alt: 'PulseChat' }),
        el('h2', { text: 'Welcome to PulseChat' }),
        el('p', { text: 'Pick a conversation from the list, or find people to connect with.' }),
      ]),
    ]),
...
function wireSocket() {
  ...
  socket.on('message:new', (p) => {
    ...
    if (chatView.isOpen(p.conversation.id)) {
      ...
      notify('PulseChat', `New message from ${name}`, `/chat/${p.conversation.id}`);
    }
    chatsView.draw();
    updateBadges();
  });
  ...
  socket.on('contact:request', (p) => {
    ...
    notify('PulseChat', `${p.from.displayName} sent you a friend request`, '/people');
  });
  ...
  socket.on('contact:accepted', (p) => {
    ...
    notify('PulseChat', `${p.contact.displayName} accepted your request`, '/chats');
  });
