...
function callsList() {
  if (!state.calls.length) {
    return el('div', { class: 'empty-state' }, [
      el('div', { class: 'empty-icon', html: icon('calls') }),
      el('h3', { text: 'No calls yet' }),
      el('p', { text: 'Start a video call with one of your contacts — calls you make and receive will appear here.' }),
    ]);
  }
  return el('div', { class: 'fade-swap' }, [
    ...state.calls.map((c) => {
