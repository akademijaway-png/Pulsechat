...
/* Open the avatar chooser: upload your own photo OR pick one of 9 built-ins. */
async function changeAvatar() {
  let meta = { builtinAvatars: [] };
  try {
    meta = await api.get('/auth/meta');
  } catch {
    /* fall through with empty list */
  }
  const builtins = meta.builtinAvatars || [];

  const grid = el('div', { class: 'avatar-grid' });
  if (builtins.length) {
    for (const av of builtins) {
      grid.append(
        el('button', {
          class: 'avatar-option',
          'data-av': av.id,
          title: av.name,
          html: `<img src="${esc(av.url)}" alt="${esc(av.name)}" loading="lazy">`,
          onclick: async () => {
            try {
              await api.post('/media/upload', { purpose: 'avatar', builtin: av.id });
              state.me = { ...state.me, avatar: av.url };
  ...
  const uploadBtn = el('button', {
    ...
    html: icon('camera') + '<span>Upload my own photo</span>',
    onclick: async () => {
      try {
  ...
  const body = el('div', {}, [
    el('p', { style: 'font-size:13px;color:var(--text-dim);margin-bottom:12px', text: 'Pick one of our avatars, or upload your own photo.' }),
    grid,
    uploadBtn,
  ]);
  openModal('Choose profile picture', body);
}
